...
func NewRouteKeahlian(db *gorm.DB, router *gin.Engine) {
	repository := repositories.NewRepositoryKeahlian(db)
	service := services.NewServiceKeahlian(repository)
	handler := handlers.NewHandlerKeahlian(service)

	route := router.Group("/api/v1/keahlian")
	route.Use(middlewares.AuthToken())
	route.Use(middlewares.AuthRole(configs.RoleConfig))

	router.GET("/api/v1/keahlian/ping", handler.HandlerPing)
