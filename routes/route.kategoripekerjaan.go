...
func NewRouteKategoriPekerjaan(db *gorm.DB, router *gin.Engine) {
	repository := repositories.NewRepositoryKategoriPekerjaan(db)
	service := services.NewServiceKategoriPekerjaan(repository)
	handler := handlers.NewHandlerKategoriPekerjaan(service)

	route := router.Group("/api/v1/kategori-pekerjaan")
	route.Use(middlewares.AuthToken())
	route.Use(middlewares.AuthRole(configs.RoleConfig))

	router.GET("/api/v1/kategori-pekerjaan/ping", handler.HandlerPing)
